    ...
    <div class="container">
        ...
        <div id="forumBox" style="display:none;">
            <div class="box">
                ...
                <div class="box-body">
                    <div id="replyInfo" style="display:none;">
                        Replying to: <span id="replyToTitle"></span>
    ...
    <script>
        ...
        async function loadMessages() {
            ...
            data.messages.filter(m=>!m.replyTo).forEach(m=>{
                let badgesHTML = (m.badges || []).map(b => `[${b}]`).join(' ');
                ...
                let html = `<div class="box message">
                    <strong>${m.title}</strong> by 
                    <span onclick="showProfile('${m.username}', event)" style="cursor:pointer; text-decoration:underline;">
                        ${m.username} (ID: ${m.user_id})
                    </span> ${roleTag} ${badgesHTML} (${m.timestamp})<br>
                    ${m.message}<br>`;
                if(isAdmin) html += `<button onclick="deleteMessage(${m.id})">Delete</button> 
                ...
                data.messages.filter(r=>r.replyTo===m.id).forEach(r=>{
                    let rBadges = (r.badges || []).map(b => `[${b}]`).join(' ');
                    let rRoleTag = r.is_admin ? '<span class="admin">[ADMIN]</span>' : (r.is_moderator ? '<span class="moderator">[MOD]</span>' : '');
                    container.innerHTML += `<div class="box reply">
                        <span onclick="showProfile('${r.username}', event)" style="cursor:pointer; text-decoration:underline;">
                            ${r.username} (ID: ${r.user_id})
                        </span> ${rRoleTag} ${rBadges} (${r.timestamp}): ${r.message}
                    </div>`;
                });
            });
        }
        ...
        async function showProfile(username, event){
            ...
            box.innerHTML = `<strong>${data.username}</strong>
                             ${data.is_admin?' <span class="admin">[ADMIN]</span>':''}
                             ${data.is_moderator?' <span class="moderator">[MOD]</span>':''}<br>
